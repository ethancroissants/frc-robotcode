#!/usr/bin/env bash
# Acuity — manufacturer's master-card prep.
#
...
#   sudo curl -fsSL \
#     https://raw.githubusercontent.com/ethancroissants/frc-robotcode/master/acuity/firmware/manufacturer-setup.sh \
#     | sudo bash
#
...
#   git clone "https://${GH_TOKEN}@github.com/ethancroissants/frc-robotcode.git" /tmp/acuity-src
#   sudo bash /tmp/acuity-src/acuity/firmware/manufacturer-setup.sh
#
...
REPO_RAW="${ACUITY_REPO_RAW:-https://raw.githubusercontent.com/ethancroissants/frc-robotcode/master}"
INSTALL_USER="${SUDO_USER:-${USER:-acuity}}"
...
if [ -n "$SCRIPT_DIR" ] && [ -x "$SCRIPT_DIR/install.sh" ]; then
  ...
  curl -fsSL "$REPO_RAW/acuity/firmware/install.sh" | bash
fi
...
if [ "${#WIFI_CONNS[@]}" -gt 0 ]; then
  ...
  if [ -t 0 ] || [ -t 1 ] || [ -e /dev/tty ]; then
    ...
    warn "no controlling tty — defaulting to 'yes' (set ACUITY_KEEP_WIFI=1 to override)"
    ANS=""
  fi
  case "${ACUITY_KEEP_WIFI:-${ANS}}" in
    n|N|no|NO|No|1)
...
log "clearing SSH host keys (clones regenerate at sshd start via the install.sh drop-in)"
# install.sh drops a systemd unit into /etc/systemd/system/ssh.service.d/
# that runs `ssh-keygen -A` as ExecStartPre whenever host keys are
# missing, so each cloned card generates its own keys the first time
# sshd starts. Stock Pi OS Bookworm does NOT do this automatically —
# without the drop-in, this rm would leave every clone with sshd
# refusing to start and the customer silently locked out.
...
log "  * boot into the AP-mode captive portal (Acuity-Setup-XXXX)"
log "  * accept the team's WiFi credentials via the captive portal"
log "  * reboot and dashboard at http://acuity-NNNN.local:8080/"
log ""
log "If you want to ship a card pre-configured for a specific team,"
log "drop a customized acuity.conf onto the FAT32 boot partition"
