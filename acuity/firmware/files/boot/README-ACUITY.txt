...
----------------------------------------------------------------
  ...
  2. Open `acuity.conf.example`, fill in TEAM / SSID / PSK,
     save it as `acuity.conf` (no .example suffix).
...
TROUBLESHOOTING
----------------------------------------------------------------
* Can't see Acuity-Setup-XXXX → wait 90s, the Pi takes a moment
...
* `acuity-NNNN.local` doesn't resolve → on Windows install
  Bonjour Print Services (free from apple.com); modern Win 10+
  has it built in.
* Lost the password / want to redo setup → delete `acuity.conf`
  from the SD card's boot partition and reboot. The Pi falls
  back to AP mode.

Default user / password (SSH): acuity / acuity
